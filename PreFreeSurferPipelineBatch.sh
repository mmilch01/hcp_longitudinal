...
get_usage_and_exit(){
    echo "usage: "
    echo "PreFreeSurferPipelineBatch.sh --Subject=\"<experiment list space separated>\" [options]"
    echo "options:"
...
main()
{
	...
	StudyFolder=/data/hcp
	Subjlist=HCA6002236_V1_MR
	...
	EnvironmentScript=/data/hcp/scripts/SetUpHCPPipeline.sh
	#/data/hcp/SetUpHCPPipeline.sh
	...
	for Subject in $Subjlist ; do
		...
		for folder in "${StudyFolder}/${Subject}/unprocessed/3T"/T1w_MPR*; do
			...
			T1wInputImages+="`ls $folder/${Subject}_T1w_MPR*.nii.gz | tail -1`"@
			numT1ws=$((numT1ws + 1))
		done
		...
		for folder in "${StudyFolder}/${Subject}/unprocessed/3T"/T2w_SPC*; do
			echo 1
			folderbase=$(basename "$folder")
			T2wInputImages+="`ls $folder/${Subject}_T2w_SPC*.nii.gz | tail -1`"@
			numT2ws=$((numT2ws + 1))
		done
