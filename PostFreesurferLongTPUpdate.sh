...
cross_dir=$1; shift
long_dir=$1; shift
if [ -n "$1" ]; then fs_template_dir=$1; shift; fi
...
if [ -n "$fs_template_dir" ]; then 
    echo cp -ar $fs_template_dir/T1w/$exp_long $long_dir/T1w/
    cp -ar $fs_template_dir/T1w/$exp_long $long_dir/T1w/
fi

#1. Copy MNINonLinear
echo cp -lr $cross_dir/MNINonLinear $long_dir/
cp -lr $cross_dir/MNINonLinear $long_dir/
#2. Copy T1w
echo cp -l $cross_dir/T1w/* $long_dir/T1w/
cp -l $cross_dir/T1w/* $long_dir/T1w/
dirs=(ACPCAlignment BiasFieldCorrection_sqrtT1wXT2w BrainExtraction_FNIRTbased xfms)
for dir in ${dirs[*]}; do
    echo cp -lr $cross_dir/T1w/$dir $long_dir/T1w/
    cp -lr $cross_dir/T1w/$dir $long_dir/T1w/
done
pushd $cross_dir/T1w; 
echo cp -ar fsaverage $long_dir/T1w/
cp -ar fsaverage $long_dir/T1w/
popd
#2. Copy T2w
mkdir -p $long_dir/T2w
echo cp -lr $cross_dir/T2w/* $long_dir/T2w
cp -lr $cross_dir/T2w/* $long_dir/T2w

#3. Copy missing files under freesurfer subject dir; do not overwrite.
echo cp -arn $cross_dir/T1w/$exp_cross/* $long_dir/T1w/$exp_long/
cp -arn $cross_dir/T1w/$exp_cross/* $long_dir/T1w/$exp_long/
